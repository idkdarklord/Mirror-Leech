...
from .modules import authorize, list, cancel_mirror, mirror_status, mirror, clone, watch, shell, eval, delete, count, leech_settings, search, rss, speedtest, usage, anilist
...
def getHerokuDetails(h_api_key, h_app_name):
    ...
    try:
        ...
        abc += f"<b>├ App Usaage:</b> {get_readable_time(AppQuotaUsed)}\n"
        abc += f"<b>└ Other Apps Usage:</b> {get_readable_time(OtherAppsUsage)}\n"
        return abc
...
def start(update, context):
    buttons = ButtonMaker()
    buttons.buildbutton("⚡️Owner⚡️", "https://t.me/Dark_LordSl")
